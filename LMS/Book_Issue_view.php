<?php // This script and data application were generated by AppGini 5.70 //
Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$x->SortFields = array(   
	    ...
	    // Fields that can be displayed in the csv file     $x->QueryFieldsCSV =
	array(            "`Book_Issue`.`id`" => "id", "`Book_Issue`.`issue_id`" =>
	"issue_id",         "IF( CHAR_LENGTH(`Users1`.`Name`), CONCAT_WS('',
	`Users1`.`Name`), '') /* Member */" => "Member",         "IF(
	CHAR_LENGTH(`Users1`.`Membership_Number`), CONCAT_WS('',
	`Users1`.`Membership_Number`), '') /* Number */" => "Number", "IF(
	CHAR_LENGTH(`books1`.`ISBN_NO`), CONCAT_WS('',   `books1`.`ISBN_NO`), '') /*
	Book Number */" => "Book_Number",         "IF(
	CHAR_LENGTH(`books1`.`Book_Title`), CONCAT_WS('',   `books1`.`Book_Title`),
	'') /* Book Title */" => "Book_Title",         "if(`Book_Issue`.`Issue_Date`,d
	ate_format(`Book_Issue`.`Issue_Date`,'%m/%d/%Y'),'')" => "Issue_Date",
	"if(`Book_Issue`.`Return_Date`,date_format(`Book_Issue`.`Return_Date`,'%m/%d/%
	Y'),'')" => "Return_Date",         "`Book_Issue`.`Status`" => "Status"     );
	// Fields that can be filtered     $x->QueryFieldsFilters = array(
	"`Book_Issue`.`id`" => "ID",         "`Book_Issue`.`issue_id`" => "Issue id",
	"IF(    CHAR_LENGTH(`Users1`.`Name`), CONCAT_WS('',   `Users1`.`Name`), '') /*
	Member */" => "Member",         "IF(
	CHAR_LENGTH(`Users1`.`Membership_Number`), CONCAT_WS('',
	`Users1`.`Membership_Number`), '') /* Number */" => "Number",         "IF(
	CHAR_LENGTH(`books1`.`ISBN_NO`), CONCAT_WS('',   `books1`.`ISBN_NO`), '') /*
	Book Number */" => "Book Number",         "IF(
	CHAR_LENGTH(`books1`.`Book_Title`), CONCAT_WS('',   `books1`.`Book_Title`),
	'') /* Book Title */" => "Book Title",         "`Book_Issue`.`Issue_Date`" =>
	"Issue Date",         "`Book_Issue`.`Return_Date`" => "Return Date",
	"`Book_Issue`.`Status`" => "Status"     );
